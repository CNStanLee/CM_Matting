% Linted
function [newBack] = GiveNewBackground(oriImg, triMap, unknownF, ...
    unknownAlpha, FThreshold, BgPath)
    %BLENDWITHNEWBACKGROUND Allows the user to select a new background
    % and blends it with the original image's foreground.
    %
    % Inputs:
    ...
    %   FThreshold - The threshold for determining definite foreground
    % in the trimap.
    % Outputs
    ...
    % Read and resize the new background to match the
    % original image's dimensions
    % m_newBack = imread(fullfile(Path, File));
    m_newBack = imread(BgPath);
    [height, width, ~ ] = size(oriImg); % Assuming oriImg is the reference
    % for dimensions
    m_newBack = imresize(m_newBack, [height, width]);
    m_newBack = double(m_newBack);
    
    % Blend the new background with the estimated foreground using 
    % the alpha matte
    for i = 1:3
        m_newBack(:, :, i) = unknownF(:, :, i) .* unknownAlpha + ...
        m_newBack(:, :, i) .* (1 - unknownAlpha);
         %m_newBack(:,:,i) = unknownF(:,:,i).*unknownAlpha(:,:) +
         % m_newBack(:,:,i).*(1-unknownAlpha(:,:));   
    end
    
    % Replace background with original image where trimap indicates 
    % definite foreground
    for a = 1:width
        for b = 1:height
    ...
    % imwrite(uint8(m_newBack), 'BlendedImage.png'); % Changed
    % filename to be more generic
    % drawnow;
