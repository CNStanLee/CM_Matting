function [Fmean, Bmean, coF, coB] = FunStatisticalAnalysis(frontImg, ...
    backImg)

    % FunStatisticalAnalysis
    % This function is used for get the mean value 
    % and then Covariance Matrix
    % MainProcess:
    
    % Inputs:
    % 1.frontImg(double matrix, (x, y, channel)) - front Img matrix
    % 2.backImg(double matrix, (x, y, channel)) - back Img matrix
    % (* drop not used *)3.unkownImg(double matrix, (x, y, channel)) -
    % unkown Img matrix
    % Outputs:
