% Linted
function [unknownAlpha, unknownF, unknownB] = Matting(unknownImg, ...
    triMap, coF, coB, Fmean, Bmean, oriVar, Iteration)
    % REFINEALPHAMATTE Refines the alpha matte of an image using
    % local averaging and iterative optimization.
    %
    % Inputs:
    %   unknownImg - The input image with unknown regions to refine 
    % the alpha matte for.
    %   triMap - The trimap of the input image, used to initialize the
    % alpha matte.
    ...
    %   oriVar - The variance of the original image used in the
    ...
    for a = 1:width
        for b = 1:height
            if any(unknownImg(a, b, :))
                ...
                [alpha, count] = LocalAverageAlpha(a, b, alpha, ...
                    count, unknownAlpha, width, height);
                ...
                for iter = 1:Iteration
                    ...
                    [tempF, tempB] = EstimateFB(a, b, alpha, invcoF, ...
                        invcoB, oriVar, Fmean, Bmean, unknownImg);
                    
                    % Calculate new alpha using estimated F and B
                    alpha = CalculateAlpha(reshape(unknownImg(a, b, :), ...
                        [3, 1]), tempF, tempB, preAlpha);
    
                    % Check for convergence (optional, depending
                    % on your criteria)
